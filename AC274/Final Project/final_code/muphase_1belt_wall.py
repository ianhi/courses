...
def mask_to_rho(mask, lx, ly):
    ...
    return rho
class LBM_1belt_wall():
    def __init__(self, G = -5, lx=130, ly=130, omega = 1,G1 = None, G2=0,
                init_type = 'rand',g = 0,g_theta=-np.pi/2, rho=None,input_rho=None,
                boundaries=None):
        self.lx = lx
        self.ly = ly
        self.G = G
        ...
        if boundaries is None:
            ...
            #can walls
            self.bound_array = np.zeros([self.lx,self.ly])
            self.bound_array[:,0] = 1
            self.bound_array[:,-1] = 1
            self.bound_array[0,:] = 1
            self.bound_array[-1,:] = 1

            self.bound_adaj = np.zeros([self.lx,self.ly])
            self.bound_adaj[:,1] = 1
            self.bound_adaj[:,-1] = 1
            self.bound_adaj[1,:] = 1
            self.bound_adaj[-1,:] = 1
            self.bound_adaj[self.bound_array==1]=0
            print(self.bound_adaj)
        else:
            self.bound_array = boundaries
        # Intiliaze rho
        if rho is not None:
            self.rho=rho
        elif init_type.lower() == 'rand':
            self.rho = 1+.05*((np.random.rand(self.lx,self.ly)-.5)*2)
        elif init_type.lower() == 'given' and input_rho is not None:
            self.rho = np.load(input_rho)
            self.lx = self.rho.shape[0]
            self.ly = self.rho.shape[1]
    ...
    def step(self,Nsteps,force_method=force_shan_chen):
        print(np.sum(self.rho[1:-1,1:-1]))
        # print(np.min(self.rho))
        for ts in range(0,Nsteps):
            ...
            # for k in range(0,9):
            #     self.f[0,k,0,:] = self.f[0,opp[k],1,:]
            #     self.f[0,k,-1,:] = self.f[0,opp[k],-2,:]
            #     self.f[0,k,:,0] = self.f[0,opp[k],:,1]
            #     self.f[0,k,:,-1] = self.f[0,opp[k],:,-2]
            # Direction 5
            # Bottom
            self.f[0,5,:,-1] = np.roll(self.f[0,7,:,-2],-1,axis=0) #shift left
            self.f[0,6,:,-1] = np.roll(self.f[0,8,:,-2],1,axis=0)
            self.f[0,2,:,-1] = self.f[0,4,:,-2]

            # WHY IS THIS BACKWARDSS?S?S??!??!??!
            self.f[0,7,:,-1] = np.roll(self.f[0,5,:,-2],1,axis=0) #shift left
            self.f[0,8,:,-1] = np.roll(self.f[0,6,:,-2],-1,axis=0)
            self.f[0,4,:,-1] = self.f[0,2,:,-2]
            ...
            # print("")
            # print(self.f[0,5,:,1])
            self.f[0,7,:,0] = np.roll(self.f[0,5,:,1],-1,axis=0) #shift left
            self.f[0,8,:,0] = np.roll(self.f[0,6,:,1],1,axis=0)
            self.f[0,4,:,0] = self.f[0,2,:,1]

            #Also BACKWARDSS?!??!?!
            self.f[0,5,:,0] = np.roll(self.f[0,7,:,1],-1,axis=0) #shift left
            self.f[0,6,:,0] = np.roll(self.f[0,8,:,1],1,axis=0)
            self.f[0,2,:,0] = self.f[0,4,:,1]
            # print(self.f[0,7,:,0])
            #left
            self.f[0,5,0,:] = np.roll(self.f[0,7,1,:],-1,axis=0)
            self.f[0,8,0,:] = np.roll(self.f[0,6,1,:],1,axis=0)
            self.f[0,1,0,:] = self.f[0,3,1,:]

            #right
            self.f[0,6,-1,:] = np.roll(self.f[0,8,-2,:],-1,axis=0)
            self.f[0,7,-1,:] = np.roll(self.f[0,5,-2,:],1,axis=0)
            self.f[0,3,-1,:] = self.f[0,1,-2,:]

            # for
                # # Bottom left corners
                # self.f[0,k,0,0] = self.f[0,opp[k],1,1]
                # self.f[0,k,0,-1] = self.f[0,opp[k],1,-2]
                # self.f[0,k,-1,0] = self.f[0,opp[k],-2,1]
                # self.f[0,k,-1,-1] = self.f[0,opp[k],-2,-2]


                # self.f[0,k][self.bound_array == 1] = self.f[0,opp[k]][self.bound_array == 1]

            # try simplest thing - points next to wall get turned around
            # for k in range(0,9):
            #     self.f[0,k][self.bound_adaj == 1] = self.f[0,opp[k]][self.bound_adaj == 1]
            for kk in range(0,9):
                #TODO Check that fout is necessary
                #TODO check that the loop is necessary. One numpy roll?
                self.f[1,kk,:,:] = np.roll(self.f[0,kk],(ex[kk],ey[kk]),(0,1))
                # self.f[1,kk,:,:] = np.roll(self.f[0,kk],(ey[kk],ex[kk]),(0,1))
            self.u[:,:] = 0
            self.v[:,:] = 0
            ...
            # print(force_x.shape)
            self.u[2:-2,2:-2] += force_x[2:-2,2:-2]/(self.omega*self.rho[2:-2,2:-2])
            self.v[2:-2,2:-2] += force_y[2:-2,2:-2]/(self.omega*self.rho[2:-2,2:-2])
