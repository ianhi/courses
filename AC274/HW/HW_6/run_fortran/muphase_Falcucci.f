...
c	   if (mod(istep,ndiag).eq.0) then
c	      call out0d
c            call energy
c            call calcsurf
c	   endif
...
clamur        open(10,file=fileout//'.uy',status='new')
        open(11,file=fileout//'.rho_u_v_1D',status='unknown')
     ...
     &        form='unformatted')
        ...
        open(55,file=fileout//'.forces_1D',status='unknown')
        open(65,file='press_field.dat',status='unknown')

        print*,'*******************************************************'
        print*,'         Lattice BGK model, 2D with 9 velocities'
        print*,'             multiphse code with 2-belt EoS'
        print*,'*******************************************************'
...
clamur	write(6,'(A)')'Output file :',fileout
...
!         write(*,*) 'popolazione_move', f(5,32,32)
...
! SHIFT EQUILIBRIO
       ...
       do i=1,nx
          do j=1,ny
        !!   write(*,*) force_x(i,j)/(omega*rhod1(i,j))
...
!         write(*,*)'i w valgono,',w(0),w(1),w(5)
     ...
     &  w(5)*(psi(i,j)+psi(i,j)+psi(i,j)+psi(i,j)))                        ! QUI ci ho messo i PESI DI SHAN-CHEN !!!!!!!!!!!!
...
c        write(*,'(6e18.10)') paramord
        ...
        densit = densit / dfloat(nx*ny)  ! noemlizzato sul bulk (non prende le cornici)
...
!         write(*,*)'il perimetro della interfaccia è fomato da...'
!         write(*,*) isurf, 'nodi !!!'
...
!                                                          !*psi(i,j)*psi(i,j)
        ...
        enddo
         ...
         write(*,*) 'rho_min=',rhomin,'rho_max=',rho_max
         write(*,*) '......................................'

         write(*,*) '..calcolo della rho media...'
         ...
         write(*,*)'A_1 vale',gnn+gnnn
         write(*,*)'A_2 vale',gnn+1.5d0*gnnn
